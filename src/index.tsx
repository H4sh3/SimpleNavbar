...
const SimpleNavbar: React.FC<SimpleNavbarProps> = ({ children, links, logo, maxMenuVh, bgColorOnHover = 'bg-black' }) => {
    ...
    return (
        <div>
            ...
            <div className="z-9 fixed w-full bg-gray-100" style={{ opacity: menuOpen ? 1 : 0, "height": menuOpen ? maxMenuVh : '0vh', "transition": "all 0.4s ease-in-out" }}>
                {
                    menuOpen ?
                        <div style={{ "height": menuOpen ? '90vh' : '0vh', "transition": "all 0.4s ease-in-out" }}>
                            ...
                            <div className="pr-2 pt-10 w-full text-center" onClick={() => { setMenuOpen(false) }}>
                                &#9650;
                            </div>
                        </div>
                        : <></>
